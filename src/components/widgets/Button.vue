...
<template>
  <div
    ref="parent"
    class="buttonresizable-component"
    :style="style"
  >
    <slot />
    <div
...
<script>
const ELEMENT_MASK = {
  "buttonresizable-r": { bit: 0b0001, cursor: "e-resize" },
  "buttonresizable-rb": { bit: 0b0011, cursor: "se-resize" },
  "buttonresizable-b": { bit: 0b0010, cursor: "s-resize" },
  "buttonresizable-lb": { bit: 0b0110, cursor: "sw-resize" },
  "buttonresizable-l": { bit: 0b0100, cursor: "w-resize" },
  "buttonresizable-lt": { bit: 0b1100, cursor: "nw-resize" },
  "buttonresizable-t": { bit: 0b1000, cursor: "n-resize" },
  "buttonresizable-rt": { bit: 0b1001, cursor: "ne-resize" },
  "buttondrag-el": { bit: 0b1111, cursor: "pointer" }
};
...
export default {
  ...
  methods: {
    ...
    setupDragElements(selector) {
      const oldList = this.$el.querySelectorAll(".buttondrag-el");
      oldList.forEach(el => {
        el.classList.remove("buttondrag-el");
      });
      const nodeList = this.$el.querySelectorAll(selector);
      nodeList.forEach(el => {
        el.classList.add("buttondrag-el");
      });
    ...
    handleMove(event) {
      if (this.resizeState !== 0) {
        ...
        if (this.resizeState & ELEMENT_MASK["buttonresizable-r"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["buttonresizable-b"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["buttonresizable-l"].bit) {
          if (!this.dragState && this.w - diffX < this.minW)
        ...
        if (this.resizeState & ELEMENT_MASK["buttonresizable-t"].bit) {
          if (!this.dragState && this.h - diffY < this.minH)
    ...
    handleDown(event) {
      if (event.target.closest && event.target.closest('.buttonresizable-component') !== this.$refs['parent'])
        return
      for (let elClass in ELEMENT_MASK) {
        if (
          ...
          elClass === "buttondrag-el" && (this.dragState = true);
...
<style scoped>
.buttonresizable-component {
  position: absolute;
}

.buttonresizable-component > .buttonresizable-r {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.buttonresizable-component > .buttonresizable-rb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.buttonresizable-component > .buttonresizable-b {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.buttonresizable-component > .buttonresizable-lb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.buttonresizable-component > .buttonresizable-l {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.buttonresizable-component > .buttonresizable-lt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.buttonresizable-component > .buttonresizable-t {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.buttonresizable-component > .buttonresizable-rt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
