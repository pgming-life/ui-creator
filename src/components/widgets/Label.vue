...
<template>
  <div
    ref="parent"
    class="labelresizable-component"
    :style="style"
  >
    <slot />
    <div
...
<script>
const ELEMENT_MASK = {
  "labelresizable-r": { bit: 0b0001, cursor: "e-resize" },
  "labelresizable-rb": { bit: 0b0011, cursor: "se-resize" },
  "labelresizable-b": { bit: 0b0010, cursor: "s-resize" },
  "labelresizable-lb": { bit: 0b0110, cursor: "sw-resize" },
  "labelresizable-l": { bit: 0b0100, cursor: "w-resize" },
  "labelresizable-lt": { bit: 0b1100, cursor: "nw-resize" },
  "labelresizable-t": { bit: 0b1000, cursor: "n-resize" },
  "labelresizable-rt": { bit: 0b1001, cursor: "ne-resize" },
  "labeldrag-el": { bit: 0b1111, cursor: "pointer" }
};
...
export default {
  ...
  methods: {
    ...
    setupDragElements(selector) {
      const oldList = this.$el.querySelectorAll(".labeldrag-el");
      oldList.forEach(el => {
        el.classList.remove("labeldrag-el");
      });
      const nodeList = this.$el.querySelectorAll(selector);
      nodeList.forEach(el => {
        el.classList.add("labeldrag-el");
      });
    ...
    handleMove(event) {
      if (this.resizeState !== 0) {
        ...
        if (this.resizeState & ELEMENT_MASK["labelresizable-r"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["labelresizable-b"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["labelresizable-l"].bit) {
          if (!this.dragState && this.w - diffX < this.minW)
        ...
        if (this.resizeState & ELEMENT_MASK["labelresizable-t"].bit) {
          if (!this.dragState && this.h - diffY < this.minH)
    ...
    handleDown(event) {
      if (event.target.closest && event.target.closest('.labelresizable-component') !== this.$refs['parent'])
        return
      for (let elClass in ELEMENT_MASK) {
        if (
          ...
          elClass === "labeldrag-el" && (this.dragState = true);
...
<style scoped>
.labelresizable-component {
  position: absolute;
}

.labelresizable-component > .labelresizable-r {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.labelresizable-component > .labelresizable-rb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.labelresizable-component > .labelresizable-b {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.labelresizable-component > .labelresizable-lb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.labelresizable-component > .labelresizable-l {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.labelresizable-component > .labelresizable-lt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.labelresizable-component > .labelresizable-t {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.labelresizable-component > .labelresizable-rt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
