...
<script>
const ELEMENT_MASK = {
  "mainresizable-r": { bit: 0b0001, cursor: "e-resize" },
  "mainresizable-rb": { bit: 0b0011, cursor: "se-resize" },
  "mainresizable-b": { bit: 0b0010, cursor: "s-resize" },
  "mainresizable-lb": { bit: 0b0110, cursor: "sw-resize" },
  "mainresizable-l": { bit: 0b0100, cursor: "w-resize" },
  "mainresizable-lt": { bit: 0b1100, cursor: "nw-resize" },
  "mainresizable-t": { bit: 0b1000, cursor: "n-resize" },
  "mainresizable-rt": { bit: 0b1001, cursor: "ne-resize" },
  "maindrag-el": { bit: 0b1111, cursor: "pointer" }
};
...
export default {
  ...
  methods: {
    ...
    setupDragElements(selector) {
      const oldList = this.$el.querySelectorAll(".maindrag-el");
      oldList.forEach(el => {
        el.classList.remove("maindrag-el");
      });
      const nodeList = this.$el.querySelectorAll(selector);
      nodeList.forEach(el => {
        el.classList.add("maindrag-el");
      });
    ...
    handleMove(event) {
      if (this.resizeState !== 0) {
        ...
        if (this.resizeState & ELEMENT_MASK["mainresizable-r"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["mainresizable-b"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["mainresizable-l"].bit) {
          if (!this.dragState && this.w - diffX < this.minW)
        ...
        if (this.resizeState & ELEMENT_MASK["mainresizable-t"].bit) {
          if (!this.dragState && this.h - diffY < this.minH)
    ...
    handleDown(event) {
      if (event.target.closest && event.target.closest('.mainresizable-component') !== this.$refs['parent'])
        return
      for (let elClass in ELEMENT_MASK) {
        if (
          ...
          elClass === "maindrag-el" && (this.dragState = true);
...
<style scoped>
.mainresizable-component {
  position: relative;
}

.mainresizable-component > .mainresizable-r {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.mainresizable-component > .mainresizable-rb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.mainresizable-component > .mainresizable-b {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.mainresizable-component > .mainresizable-lb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.mainresizable-component > .mainresizable-l {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.mainresizable-component > .mainresizable-lt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.mainresizable-component > .mainresizable-t {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.mainresizable-component > .mainresizable-rt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
