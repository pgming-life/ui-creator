<!-- MIT License vue-resizable -->
<!-- https://github.com/nikitasnv/vue-resizable/blob/master/LICENSE -->

<template>
  <div
    ref="parent"
    class="inputboxresizable-component"
    :style="style"
  >
    <slot />
    <div
...
<script>
const ELEMENT_MASK = {
  "inputboxresizable-r": { bit: 0b0001, cursor: "e-resize" },
  "inputboxresizable-rb": { bit: 0b0011, cursor: "se-resize" },
  "inputboxresizable-b": { bit: 0b0010, cursor: "s-resize" },
  "inputboxresizable-lb": { bit: 0b0110, cursor: "sw-resize" },
  "inputboxresizable-l": { bit: 0b0100, cursor: "w-resize" },
  "inputboxresizable-lt": { bit: 0b1100, cursor: "nw-resize" },
  "inputboxresizable-t": { bit: 0b1000, cursor: "n-resize" },
  "inputboxresizable-rt": { bit: 0b1001, cursor: "ne-resize" },
  "inputboxdrag-el": { bit: 0b1111, cursor: "pointer" }
};
...
export default {
  ...
  methods: {
    ...
    setupDragElements(selector) {
      const oldList = this.$el.querySelectorAll(".inputboxdrag-el");
      oldList.forEach(el => {
        el.classList.remove("inputboxdrag-el");
      });
      const nodeList = this.$el.querySelectorAll(selector);
      nodeList.forEach(el => {
        el.classList.add("inputboxdrag-el");
      });
    ...
    handleMove(event) {
      if (this.resizeState !== 0) {
        ...
        if (this.resizeState & ELEMENT_MASK["inputboxresizable-r"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["inputboxresizable-b"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["inputboxresizable-l"].bit) {
          if (!this.dragState && this.w - diffX < this.minW)
        ...
        if (this.resizeState & ELEMENT_MASK["inputboxresizable-t"].bit) {
          if (!this.dragState && this.h - diffY < this.minH)
    ...
    handleDown(event) {
      if (event.target.closest && event.target.closest('.inputboxresizable-component') !== this.$refs['parent'])
        return
      for (let elClass in ELEMENT_MASK) {
        if (
          ...
          elClass === "inputboxdrag-el" && (this.dragState = true);
...
<style scoped>
.inputboxresizable-component {
  position: absolute;
}

.inputboxresizable-component > .inputboxresizable-r {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.inputboxresizable-component > .inputboxresizable-rb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.inputboxresizable-component > .inputboxresizable-b {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.inputboxresizable-component > .inputboxresizable-lb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.inputboxresizable-component > .inputboxresizable-l {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.inputboxresizable-component > .inputboxresizable-lt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.inputboxresizable-component > .inputboxresizable-t {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.inputboxresizable-component > .inputboxresizable-rt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
