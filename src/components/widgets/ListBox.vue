...
<template>
  <div
    ref="parent"
    class="listboxresizable-component"
    :style="style"
  >
    <slot />
    <div
...
<script>
const ELEMENT_MASK = {
  "listboxresizable-r": { bit: 0b0001, cursor: "e-resize" },
  "listboxresizable-rb": { bit: 0b0011, cursor: "se-resize" },
  "listboxresizable-b": { bit: 0b0010, cursor: "s-resize" },
  "listboxresizable-lb": { bit: 0b0110, cursor: "sw-resize" },
  "listboxresizable-l": { bit: 0b0100, cursor: "w-resize" },
  "listboxresizable-lt": { bit: 0b1100, cursor: "nw-resize" },
  "listboxresizable-t": { bit: 0b1000, cursor: "n-resize" },
  "listboxresizable-rt": { bit: 0b1001, cursor: "ne-resize" },
  "listboxdrag-el": { bit: 0b1111, cursor: "pointer" }
};
...
export default {
  ...
  methods: {
    ...
    setupDragElements(selector) {
      const oldList = this.$el.querySelectorAll(".listboxdrag-el");
      oldList.forEach(el => {
        el.classList.remove("listboxdrag-el");
      });
      const nodeList = this.$el.querySelectorAll(selector);
      nodeList.forEach(el => {
        el.classList.add("listboxdrag-el");
      });
    ...
    handleMove(event) {
      if (this.resizeState !== 0) {
        ...
        if (this.resizeState & ELEMENT_MASK["listboxresizable-r"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["listboxresizable-b"].bit) {
        ...
        if (this.resizeState & ELEMENT_MASK["listboxresizable-l"].bit) {
          if (!this.dragState && this.w - diffX < this.minW)
        ...
        if (this.resizeState & ELEMENT_MASK["listboxresizable-t"].bit) {
          if (!this.dragState && this.h - diffY < this.minH)
    ...
    handleDown(event) {
      if (event.target.closest && event.target.closest('.listboxresizable-component') !== this.$refs['parent'])
        return
      for (let elClass in ELEMENT_MASK) {
        if (
          ...
          elClass === "listboxdrag-el" && (this.dragState = true);
...
<style scoped>
.listboxresizable-component {
  position: absolute;
}

.listboxresizable-component > .listboxresizable-r {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.listboxresizable-component > .listboxresizable-rb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.listboxresizable-component > .listboxresizable-b {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.listboxresizable-component > .listboxresizable-lb {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.listboxresizable-component > .listboxresizable-l {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.listboxresizable-component > .listboxresizable-lt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
...
.listboxresizable-component > .listboxresizable-t {
  display: block;
  position: absolute;
  z-index: 90;
  touch-action: none;
...
.listboxresizable-component > .listboxresizable-rt {
  display: block;
  position: absolute;
  touch-action: none;
  user-select: none;
